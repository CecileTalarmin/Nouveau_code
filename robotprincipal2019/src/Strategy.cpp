...
using namespace std;
//Constructeur
Strategy::Strategy(std::string path){
...
int Strategy::lireStrat() {
    //lit la strategie, et l'enregistre dans la classe
    std::ifstream fichier(nameFileStrat.c_str());
    std::string line;

    while(getline(fichier, line)) {
        if(line.find("(") != std::string::npos){//npos est la valeur renvoyee si la valeur n'est pas trouvee
...
int Strategy::parserLigneChoix(string line) {
    ...
    while((unsigned)(indiceParentheseOuvrante = line.find("(", indiceParentheseOuvrante+1)) != string::npos) {
        ...
        if(statutParser.compare(statutCouleurs) == 0 || statutParser.compare(statut) == 0 || statutParser.compare("default") == 0) {
            ...
            if(indice != string::npos){
                ...
                std::string substr = line.substr(indiceParentheseFermante, indice-indiceParentheseFermante+1);
                ...
                if(substr.find(".json") != string::npos) {
                    ...
                    std::string nomStrategie =  line.substr(indiceParentheseFermante+3, indice-5-indiceParentheseFermante);
...
string Strategy::getStatut() {
    return statut;
}

void Strategy::setStatut(string stat){
    statut = stat;
}

void Strategy::setStatutCouleurs(string statCoul) {
    statutCouleurs = statCoul;
}

unsigned int Strategy::getNbObjectifs() {
    return nbObjectifs;
}

Point Strategy::getPointActuel() {
    //cout << "Point actuel : "; points[indexLigne].display();
    return points[indexLigne];
}

//renvoie le point actuel si plus de point
Point Strategy::getPointSuivant() {
    loin = false;
...
string Strategy::getFileAction() {
    return getPointActuel().getAction();
}

int Strategy::getSensDeplacement() {
    return getPointActuel().getSens();
}

int Strategy::getOptionBlocage() {
    return getPointActuel().getBlocage();
}

bool Strategy::haveToWaitActionDone() {
    return getPointActuel().getAttAction();
}

unsigned int Strategy::getTimeOut() {
    return (unsigned int)getPointActuel().getTimeOut();
}

string Strategy::getNameFileStrat() {
    return nameFileStrat;
}
string Strategy::getNameFilePoint() {
    return nameFilePoint;
}

bool Strategy::isNotFinished(){
    return !finished;
}
void Strategy::incrNbObjectifs() {
    nbObjectifs++;
    return;
}

void Strategy::pushInteruption() {
...
Point Strategy::eloigner(Point pt, int distanceMM) {
...
bool Strategy::estEloigne() {
    return loin;
}

void Strategy::setVitesse(int vit) {
